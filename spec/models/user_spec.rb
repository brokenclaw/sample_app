...
describe User do
    ...
    it "should not accept invalid email addresses" do
        ...
        addresses.each do |address|
            valid_email_usr = User.new(@attr.merge(:email=>address))
            valid_email_usr.should_not be_valid
        end
    end
    ...
    it "should reject duplicated email addresses caseinsensitive" do
        User.create!(@attr)
        user_with_duplicated_email = User.new(@attr.merge(:email=>@attr[:email].upcase))
        user_with_duplicated_email.should_not be_valid
    end
